[(${class.getVisibility().toString()})] [# th:if="${class.isAbstract()}"]abstract [/]class [(${class.getName()})][# th:if="${class.getSuperTypes().size()} gt 0"] extends [# th:each="type,iterStat : ${class.getSuperTypes()}"][(${type.getName()})][# th:unless="${iterStat.last}"], [/][/][/][# th:if="${class.getImplementedInterfaces().size()} gt 0"]
